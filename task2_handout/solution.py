...
'''
        ...
        x = F.dropout(F.relu(self.fc1(x)), p=self.dropout_p,
                training=self.training or self.dropout_at_eval)
        
        x = F.dropout(F.relu(self.fc2(x)), p=self.dropout_p,
                training=self.training or self.dropout_at_eval)
        x = self.fc3(x)
    ...
    def train(self):
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                #batch_x=torch.reshape(batch_x, (self.batch_size, 1, 28, 28))
                #batch_y=torch.reshape(batch_y, (self.batch_size, 1, 1))
                self.network.zero_grad()

                # TODO: MC_Dropout_2. Implement MCDropout training here
                # You need to calculate the loss based on the literature
                
                #reg_constant=0.0001-> ECE=0.053
                
                reg_constant=0.0002

                #pred=self.network.forward(batch_x)
                cross_entropy=nn.CrossEntropyLoss()
                l2_norm = sum(p.pow(2.0).sum() for p in self.network.parameters())
                
                pred=self.network.forward(batch_x)
                loss = cross_entropy(pred, batch_y)+reg_constant*l2_norm
                # Backpropagate to get the gradients
                loss.backward()
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        for i in range(num_sample):
            ...
            if i==0:
                ...
                estimated_probability=torch.nn.functional.softmax(output)
