...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    layers = []
    # all layers before output layer
    for layer_in, layer_out in zip(sizes[:-2], sizes[1:-1]):
        layers += [
...
class Actor(nn.Module):
    ...
    def _distribution(self, obs):
        ...
        pi = torch.distributions.categorical.Categorical(
            logits=logits
            )
        return pi
    ...
    def forward(self, obs, act=None):
        ...
        log_prob = (
            self._log_prob_from_distribution(pi, act) if act is not None
            else None
            )
...
class VPGBuffer:
    ...
    def store(self, obs, act, rew, val, logp):
        ...
        # DONE: Store new data in the respective buffers.
    ...
    def end_traj(self, last_val=0):
        ...
        '''self.tdres_buf[path_slice] = (
            self.rew_buf[path_slice] + self.gamma * vals[1:]
            )'''

        v_new = vals[1:]
        v_old = vals[:-1]
        diff = rews[:-1] + v_new- v_old

        self.tdres_buf[path_slice] = discount_cumsum(diff, self.gamma*self.lam)
        
        # TODO: discount_cumsum??? I think I'm implementing the more unstable version
        # with just 1 observation


        # TODO: Implement discounted rewards-to-go calculation. 
        # Hint: use the discount_cumsum function 
        self.ret_buf[path_slice] = discount_cumsum(
            x=rews, discount=self.gamma
            )[:-1]
        # TODO: isn't this buffer already implemented above???
...
class Agent:
    ...
    def step(self, state):
        ...
        # `torch.no_grad` to ensure that it does not interfer with the gradient
        # computation.
        with torch.no_grad():
    ...
    def get_action(self, obs):
        ...
        with torch.no_grad():
            ...
            pi, log_prob = self.actor.forward(obs, act=None)
            # get best action
            act = pi.probs.argmax(axis=-1)
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        # base loss:
        # optimize E_tau~p_theta[R(tau)D_theta(sum of logp)] where
        # R(tau) is the reward of the path tau
        R = data["ret"]  # rewards to go at each point of the path
        _, logp = agent.actor.forward(obs=data["obs"], act=data["act"])
        loss_actor = -torch.mul(R, logp).sum()
        loss_actor.backward()
        ...
        for _ in range(100):
            ...
            # TD-loss
            V = agent.critic.forward(obs=data["obs"])
            loss_critic = torch.norm(V - data["tdres"]) / (2 * V.size(dim=0))
